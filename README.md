...
| [taichu-eval-reproduction](https://github.com/GuoCheng24/taichu-eval-reproduction) | Two ZDTaichu5.0-9B model-card numbers re-measured on one RTX 4090, on the card's protocol (thinking on, LLM answer extraction). Thinking mode was affordable on a random subsample, so four estimators of the full-set accuracy are reported rather than one: CV-Bench lands **88.45%** to **89.33%**, above the card's 86.82; MathVista **77.20%** to **82.00%**, below 84.50. Every interval contains its card number — the regression estimator, the tightest, gives **79.75%** [73.6, 85.9] on MathVista. Off the protocol, 82.4% and 73.2% on the full sets. |
